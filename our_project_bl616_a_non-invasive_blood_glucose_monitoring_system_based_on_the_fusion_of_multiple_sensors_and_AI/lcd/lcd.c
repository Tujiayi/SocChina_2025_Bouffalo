...
void lcd_draw_curve(uint16_t x, uint16_t y, uint16_t w, uint16_t h, 
                    uint16_t *data, uint16_t num, uint16_t min, uint16_t max, lcd_color_t color)
{
    if (num < 2) return;
    uint16_t prev_x = x;
    // 映射第一个点
    uint16_t prev_y = y +  h - (data[0] - min) * h / (max - min + 1);
    for (uint16_t i = 1; i < num; i++) {
        uint16_t cur_x = x + i * w / (num - 1);
        uint16_t cur_y = y +  h - (data[i] - min) * h / (max - min + 1);
        lcd_draw_area(cur_x, cur_y, cur_x , cur_y , 0x0000); // 
        lcd_draw_line(prev_x, prev_y, cur_x, cur_y, color);
        prev_x = cur_x;
        prev_y = cur_y;
    }
}
...
// 支持中文显示的函数
int lcd_draw_str_ascii16_with_chinese(uint16_t x, uint16_t y, lcd_color_t color, 
                                     lcd_color_t bk_color, uint8_t *str, uint16_t num)
{
    static lcd_color_t draw_buff[2][16 * 16]; // 增加缓冲区大小以支持16x16汉字
    uint16_t buff_color_num;
    uint8_t buff_using_num = 0;
    uint8_t ch;
    uint16_t x0 = x;
    ...
    for (uint16_t i = 0; i < num && str[i]; i++) {
        ...
        if (ch < 0x80) {
            ...
            buff_color_num = 0;
            for (uint8_t j = 0; j < 16; j++) {
                uint8_t temp = font_ascii_16x8[ch * 16 + j];
                for (uint8_t k = 0; k < 8; k++) {
                    if (temp & (0x80 >> k))
                        draw_buff[buff_using_num][buff_color_num++] = color;
                    else
        ...
        // 处理GB2312中文字符 (0xA1-0xF7 + 0xA1-0xFE)
        else if (i + 1 < num && ch >= 0xA1 && ch <= 0xF7) {
            if (x > lcd_max_x - 16) {
                x = x0;
                y += 16;
            }
            if (x > lcd_max_x - 16 || y > lcd_max_y - 16)
                break;
            
            uint8_t next_ch = str[i + 1];
            i++; // 消耗两个字节
            
            // 计算GB2312编码 (0xA1A1 到 0xF7FE)
            uint16_t gb_code = (ch << 8) | next_ch;
            
            // 在中文点阵库中查找字符
            const uint8_t* char_data = find_chinese_char(gb_code);
            if (!char_data) continue; // 未找到字符数据
            
            // 准备左边8像素
            buff_color_num = 0;
            for (uint8_t line = 0; line < 16; line++) {
                uint8_t left_byte = char_data[line * 2]; // 左边8位数据
            ...
            buff_color_num = 0;
            for (uint8_t line = 0; line < 16; line++) {
                uint8_t right_byte = char_data[line * 2 + 1]; // 右边8位数据
                for (uint8_t bit = 0; bit < 8; bit++) {
            ...
            while (lcd_draw_is_busy()) {};
            lcd_draw_picture_nonblocking(x + 8, y, x + 15, y + 15, draw_buff[buff_using_num]);
            buff_using_num = !buff_using_num;
...
// 中文点阵查找函数
const uint8_t* find_chinese_char(uint16_t gb_code)
{
    // 实际项目中应使用二分查找或哈希表
    for (uint16_t i = 0; i < CHINESE_FONT_SIZE; i++) {
        if (chinese_font_16x16[i].code == gb_code) {
            return chinese_font_16x16[i].data;
        }
    }
    return NULL;
}
...
const chinese_char_t chinese_font_16x16[CHINESE_FONT_SIZE] = {
    {0xA1E3, {0x00,0x00,0x18,0x00,0x24,0x00,0x24,0x00,0x18,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
              0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}},/*"°"*/
    {0xD0C4, {0x00,0x00,0x02,0x00,0x01,0x00,0x00,0x80,0x00,0x80,0x04,0x00,0x04,0x08,0x24,0x04,
              0x24,0x04,0x24,0x02,0x44,0x02,0x44,0x12,0x84,0x10,0x04,0x10,0x03,0xF0,0x00,0x00}},/*"心"*/
    {0xC2CA, {0x02,0x00,0x01,0x00,0x7F,0xFC,0x02,0x00,0x44,0x44,0x2F,0x88,0x11,0x10,0x22,0x48,
              0x4F,0xE4,0x00,0x20,0x01,0x00,0xFF,0xFE,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00}},/*"率"*/
    {0xCEC2, {0x00,0x00,0x23,0xF8,0x12,0x08,0x12,0x08,0x83,0xF8,0x42,0x08,0x42,0x08,0x13,0xF8,
              0x10,0x00,0x27,0xFC,0xE4,0xA4,0x24,0xA4,0x24,0xA4,0x24,0xA4,0x2F,0xFE,0x00,0x00}},/*"温"*/
    {0xCAAA, {0x00,0x00,0x27,0xF8,0x14,0x08,0x14,0x08,0x87,0xF8,0x44,0x08,0x44,0x08,0x17,0xF8,
              0x11,0x20,0x21,0x20,0xE9,0x24,0x25,0x28,0x23,0x30,0x21,0x20,0x2F,0xFE,0x00,0x00}},/*"湿"*/
    {0xB6C8, {0x01,0x00,0x00,0x80,0x3F,0xFE,0x22,0x20,0x22,0x20,0x3F,0xFC,0x22,0x20,0x22,0x20,
              0x23,0xE0,0x20,0x00,0x2F,0xF0,0x24,0x10,0x42,0x20,0x41,0xC0,0x86,0x30,0x38,0x0E}},/*"度"*/           
    {0xD1AA, {0x01,0x00,0x01,0x00,0x02,0x00,0x3F,0xF8,0x24,0x48,0x24,0x48,0x24,0x48,0x24,0x48,
              0x24,0x48,0x24,0x48,0x24,0x48,0x24,0x48,0x24,0x48,0x24,0x48,0xFF,0xFE,0x00,0x00}},/*"血"*/
    {0xD1F5, {0x20,0x00,0x3F,0xFC,0x40,0x00,0x9F,0xF0,0x00,0x00,0x7F,0xF0,0x11,0x10,0x0A,0x10,
              0x7F,0xD0,0x04,0x10,0x3F,0x90,0x04,0x12,0xFF,0xEA,0x04,0x0A,0x04,0x06,0x04,0x02}},/*"氧"*/
    {0xCCC7, {0x10,0x20,0x10,0x10,0x95,0xFE,0x55,0x10,0x59,0x7C,0x11,0x14,0xFD,0xFE,0x31,0x14,
