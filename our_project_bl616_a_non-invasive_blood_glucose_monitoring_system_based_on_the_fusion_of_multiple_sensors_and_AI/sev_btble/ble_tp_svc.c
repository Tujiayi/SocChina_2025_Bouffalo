...
extern uint32_t ble_flag;
extern uint8_t *blood_sugar_ble[5];
uint8_t * Location_1 ;
uint8_t * Location_2 ;
uint8_t * Location_3 ;
uint8_t len;
                                        ...
                                        const void *buf, u16_t len, u16_t offset, u8_t flags)
{
    ...
    memcpy(Ble_Buffer, buf, 50);
    BT_WARN("recv copy data:%s\r\n", Ble_Buffer);
    if (wifi_strat_flag == 2) {
        BT_WARN("other data.\r\n");
        if (strstr(Ble_Buffer,"bloodsugar") && ble_flag == 1) {
            Location_1 = strstr(Ble_Buffer, "bloodsugar");
            Location_2 = strstr(Ble_Buffer, "###");
            len = Location_2 - Location_1 - 10;
            memcpy(blood_sugar_ble, Location_1 + 10, len);
            printf("%s\r\n", blood_sugar_ble);
            ble_flag = 0;
        }
    } else if(strstr(Ble_Buffer,"WIFI_PASSID") && wifi_strat_flag == 1){
